...
1. Move each `Prefix_Class_Name` class to a namespaced `RV\Legacy\...` PSR-4 class under `lib/RV/Legacy/`, registered via the existing `"RV\\": "lib/RV/"` composer PSR-4 autoload (run `composer dump-autoload` after adding files).
   - Namespace mapping is FILE-PATH based, not class-name based: `lib/OA/Dal/Foo.php` → `RV\Legacy\OA\Dal\Foo`, regardless of the legacy class's prefix. This covers non-conforming names like `DataGenerator` (`lib/OA/Dal/DataGenerator.php` → `RV\Legacy\OA\Dal\DataGenerator`) and `Openads_Links` (`lib/OA/Dal/Links.php` → `RV\Legacy\OA\Dal\Links`); the `class_alias` shim still maps the exact old class name.
...
3. Old class names must also stay resolvable via autoload (some callers rely on `class_exists('OA_Foo')` or lazy loading without an explicit require): register the shim files in a composer `classmap` (or `files`) autoload entry so legacy underscore names resolve without an explicit include. PSR-4 alone cannot resolve legacy names.
   ...
   - Each child also adds a matching `"exclude-from-classmap": ["lib/<module>/tests/"]` entry (same one-line-per-module convention) so module test fixtures do not become globally autoloadable.
   - Verify classmap detection: composer's classmap generator scans for class-like declarations, so a shim containing only a bare `class_alias(...)` call may not be indexed under the legacy name. After `composer dump-autoload`, check `lib/vendor/composer/autoload_classmap.php` actually maps each legacy class name to its shim file. If it does not, fall back to registering the shim files under composer `"files"` autoload (eager include; acceptable for these thin shims), and note the choice in the PR.
...
7. Audit name-string-driven usages before converting each class: `class_alias` is NOT transparent to `get_class()`, `__CLASS__`, `serialize()`d class names, or string manipulation of class names (e.g. `str_replace('OA_Dal_', ...)`, `strtolower(get_class(...))` used to build table/DAL names). After migration these return/see the new namespaced name. Grep each class's dependents for `get_class`, `__CLASS__`, dynamic `new $name`, and hard-coded legacy-name string comparisons; if behavior would change and cannot be preserved trivially, leave that class unconverted and note it in the PR.
   ...
   7b. (sub-item of 7) Procedural files are NOT renamed or namespaced from their file names: names like `lib/max/Delivery/XML-RPC.php` (hyphen) and `limitations.delivery.php` (dots) stay at their exact old paths as shim entry points; if their functions are grouped into a namespaced class (recipe step 6), the new class file gets a clean PSR-4-valid name chosen by the child (e.g. `lib/RV/Legacy/Max/Delivery/XmlRpc.php`), documented in the PR. Note `lib/max` (lowercase) maps to namespace segment `Max`.
