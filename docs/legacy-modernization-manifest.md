...
2. Leave a thin back-compat shim at the old path: the old file `require`s nothing new at runtime beyond the autoloader and declares `class_alias(RV\Legacy\...::class, 'Old_Class_Name');` so all untouched callers keep working unchanged.
...
5. Procedural files (no classes — e.g. `lib/max/Delivery/*.php` delivery hot path): group functions into namespaced classes with static methods and keep thin global-function shims delegating to them, OR leave untouched if the risk/benefit is poor. No behavior or performance regressions allowed on the delivery path.
