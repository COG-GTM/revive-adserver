...
class DataGenerator
{
    ...
    {
        $ids = self::generate($do, 1, $generateParents);
    ...
    public static function generate($do, $numberOfCopies = 1, $generateParents = false)
    {
        ...
        if ($generateParents) {
            self::generateParents($do);
        }
        $doOriginal = clone ($do);
        self::setDefaultValues($do);
        self::trackData($do->getTableWithoutPrefix());

        $ids = [];
        for ($i = 0; $i < $numberOfCopies; $i++) {
            $id = $do->insert();
            $do = clone ($doOriginal);
            self::setDefaultValues($do, $i + 1);
            $ids[] = $id;
        }
        return $ids;
    }
    ...
    public static function generateParents(&$do)
    {
        ...
        foreach ($links as $foreignKey => $linkedTableField) {
            ...
            $do->$foreignKey = self::addAncestor($table, $linkedPrimaryKeyVal);
        }
    }
    ...
    public static function addAncestor($table, $primaryKey = null)
    {
        ...
        self::setDefaultValues($doAncestor);
        ...
        foreach ($links as $foreignKey => $linkedTableField) {
            ...
            $doAncestor->$foreignKey = self::addAncestor($ancestorTable);
        }
        self::trackData($table);
        $id = $doAncestor->insert();
        self::getReferenceId($table, $id); // store the id
        return $id;
    }
    ...
    public static function setDefaultValues($do, $counter = 0)
    {
        ...
        foreach ($fields as $fieldName => $fieldType) {
            if (!isset($do->$fieldName)) {
                $fieldValue = self::getFieldValueFromDataContainer($table, $fieldName, $counter);

                if (!isset($fieldValue) && !in_array($fieldName, $keys)) {
                    $fieldValue = self::defaultValueForObject($do, $fieldName, $fieldType);
                    if (!isset($fieldValue)) {
                        $fieldValue = self::defaultValueByType($fieldType);
                    }
                }
