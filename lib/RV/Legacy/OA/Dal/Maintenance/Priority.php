...
namespace RV\Legacy\OA\Dal\Maintenance;
 ...
 */
class Priority extends \RV\Legacy\OA\Dal\Maintenance\Common
{
    /**
     ...
     */
    public function setMaintenancePriorityLastRunInfo($oStart, $oEnd, $oUpdateTo, $type)
    {
     ...
     */
    public function getMaintenancePriorityLastRunInfo($type = null, $aAdditionalFields = [])
    {
        $aConf = $GLOBALS['_MAX']['CONF'];
        $table = $aConf['table']['log_maintenance_priority'];
        $aFields = ['operation_interval'];
     ...
     */
    public static $ZONE_FORECAST_DEFAULT_ZONE_IMPRESSIONS = 10;
     ...
     */
    public static $ZONE_FORECAST_DEFAULT_ZONE_IMPRESSIONS_FRACTION = 0.05;
     ...
     */
    public function getZoneForecastDefaultZoneImpressions()
    {
     ...
     * @return int
     */
    public function getZoneForecastDefaultZoneImpressionsLimit()
    {
     ...
     *               for that placement.
     */
    public function getAgencyEcpmRemnantCampaignsDeliveriesToDate($id)
    {
     ...
     *               for that placement.
     */
    public function getAgencyEcpmContractCampaignsDeliveriesToDate($id, $priority)
    {
     ...
     *               for that placement.
     */
    public function getAgencyPriorityCampaignsDeliveriesToDate($id, $priority)
    {
        $priority = (int) $priority;
        $table = $this->_getTablenameUnquoted('campaigns');
        $do = \OA_Dal::factoryDO('Campaigns');
     ...
     *               for that campaign.
     */
    public function getAgencyCampaignsDeliveriesToDate($id, $aWheres = [])
    {
        $debugDetailQuery = '';
     ...
     *               for that placement.
     */
    public function getCampaignDeliveryToday($id, $today)
    {
     ...
     *     )
     */
    public function getCampaignStats($id, $todayOnly = false, $today = '')
    {
     ...
     */
    public function &getAllZonesWithAllocInv()
    {
     ...
     */
    public function &getZonesForecastsForAllZones()
    {
     ...
     *               'start_run' date.
     */
    public function &getAllDeliveryLimitationChangedCreatives($aLastRun)
    {
        $aConf = $GLOBALS['_MAX']['CONF'];
    ...
    public function &getPreviousAdDeliveryInfo($aCurrentZones)
    {
        ...
        $aPastDeliveryResult = [];
        $aPastPriorityResult = [];
        $aNonDeliveringPastPriorityResult = [];
        $aFinalResult = [];
        ...
        $aNotInLastOIPastPriorityResult = [];
     ...
     */
    public function _calculateAveragePastPriorityValues(&$aPastPriorityResult, $aResult, $oDate, $aPastDeliveryResult = null)
    {
     ...
     */
    public function updatePriorities(&$aData)
    {
     ...
     */
    public function getRequiredAdImpressions($aAdvertID)
    {
        if (empty($aAdvertID)) {
            return [];
        }
     ...
     */
    public function getRequiredAdZoneImpressions($aAdvertID)
    {
        if (empty($aAdvertID)) {
            return [];
        }
     ...
     *               )
     */
    public function getAdZoneAssociationsByAds($aAdvertID)
    {
        ...
        $aResult = [];
        foreach ($result as $row) {
            $aResult[$row['ad_id']][] = ['zone_id' => $row['zone_id']];
        }
        return $aResult;
    }
     ...
     */
    public function saveAllocatedImpressions($aData)
    {
     ...
     */
    public function batchInsert($tableNameUnquoted, $data, $fields)
    {
     ...
     *                   )
     */
    public function getPreviousWeekZoneForcastImpressions($zoneId)
    {
     ...
     */
    public function obtainPriorityLock()
    {
     ...
     */
    public function releasePriorityLock()
    {
        if (empty($this->oLock)) {
